...
//        db.execSQL("updata ",new String[]{question.getTitleId(),question.getCollect(),question.getDelete(),question.getHasDo()});
    ...
    public List<String> queryAllTitleID(String id) {
        ...
        LogUtil.d(TAG, "-----id：" + id + "----type:" + type + "----grade:" + grade);
//        如果id不在grade和type的范围，就按照grade和type中的最小的开始调用
        if(!isCheckRange(type,grade,id)){
//            id 为最小的id,
            id = minId(type,grade);
        }
    ...
    public String minId(int type,int grade){
        Cursor cursor = db.rawQuery("select min(f_titleID) as minId from tb_title where f_type=? and f_grade=?",new String[]{""+type,""+grade});
        String minId = "";
        if(cursor.moveToFirst()){
            do {
                minId  = cursor.getString(cursor.getColumnIndex("minId"));
            }while (cursor.moveToNext());
        }
        if(cursor!=null){
            cursor.close();
     ...
     */
    public boolean isCheckRange(int type,int grade,  String currentId) {
        Cursor cursor = db.rawQuery("select max(f_titleID) as maxId,min(f_titleID) as minId from tb_title where f_type=? and f_grade=?", new String[]{"" + type, "" + grade});
        String maxId = "";
        String minId = "";
        if (cursor.moveToFirst()) {
            do {
                maxId = cursor.getString(cursor.getColumnIndex("maxId"));
                minId = cursor.getString(cursor.getColumnIndex("minId"));
            } while (cursor.moveToNext());
        }
        if (cursor != null) {
...
//        if (currentId.compareTo(maxId) <= 0 && currentId.compareTo(minId) >= 0) {
//            return true;
//        }

        int max = getIntegerId(maxId);
        int min = getIntegerId(minId);
        int curr = getIntegerId(currentId);
